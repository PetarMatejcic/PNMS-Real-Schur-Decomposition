function [H, Z] = francis_qr(H, flag = 1)
    ...
    % Računanje s i t
    if flag == 1
    % U redovnom slučaju računamo koeficijente karakterističnog polinoma donjeg desnog 2x2 bloka
        s = H(m, m) + H(n, n);
        t = H(m, m)*H(n, n) - H(m, n)*H(n, m);
    else
    % Inače uzimamo izvanredan pomak
        mu = H(n, n) + 0.5*abs(H(n, n-1));
        s = 2*mu;
        t = mu^2;
     end
    ...
    % Za Hessenbergovu matricu on ima samo prva tri elementa ~= 0 pa samo njih računamo
